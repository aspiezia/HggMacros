...
void CreateTree(bool save=false, TString sample="prova", TString sel="prova", TString variable_string="mass", int nbin=80, int nbin_min=100, int nbin_max=180)
{
  ...
  TFile *f1 = new TFile("/afs/cern.ch/work/a/aspiezia/Analysis_legacy/CMSSW_6_1_1/src/h2gglobe-legacy_paper_8TeV_optimization_v1/AnalysisScripts/bkg2.root");    //BACKGROUND
  ...
  TFile *f3 = new TFile("/afs/cern.ch/work/a/aspiezia/Analysis_legacy/CMSSW_6_1_1/src/h2gglobe-legacy_paper_8TeV_optimization_v1/AnalysisScripts/signal2.root"); //SIGNAL
  ...
  for(unsigned int j=0; j<names_bkg.size(); j++){
    TTree *s_bkg   = (TTree*)f1->Get(names_bkg[j]);
