...
int MakeFit(bool save=false, TString sel="prova"){
	...
	TFile *mass_CS  = new TFile("mass_CS.root");
	if(sel=="cat6"){
		TFile *mass_SS  = new TFile("mass_SScat6.root");
		TFile *dipho_SS = new TFile("diphobdt_output_SScat6.root");
	}
	if(sel=="cat7"){
	...
	params->Print("V");
	if(save) c->SaveAs("fit_diphoMVA.png");
	...
	DEN3->Draw("E");
	if(save){
		//TString rootfile_name = "effDiphoMVA"+sel+"_"+".root";
		//TFile fileTosave(rootfile_name,"recreate"); 
		//DEN3->Write();
		c6->SaveAs("effDiphoMVA.png");
    }
